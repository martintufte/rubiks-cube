...
def corner_trace(state: CubeState) -> str:
    ...
    for corner_idxs in corners.values():
        corner_idxs_copy = corner_idxs.copy()

        cycle = 0
        while corner_idxs_copy[0] not in explored_corners:
            explored_corners.update(set(corner_idxs))
            corner_idxs_copy = list(state[corner_idxs])
            cycle += 1
...
def edge_trace(state: CubeState) -> str:
    ...
    for edge_idxs in edges.values():
        edge_idxs_copy = edge_idxs.copy()

        cycle = 0
        while edge_idxs_copy[0] not in explored_edges:
            explored_edges.update(set(edge_idxs))
            cycle += 1
            edge_idxs_copy = list(state[edge_idxs])
