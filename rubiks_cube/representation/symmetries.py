...
def _canonicalize_group(group: dict[Symmetry, list[str]]) -> dict[Symmetry, list[str]]:
    return {
        subset: list(get_canonical_sequence(state_from_sequence(sequence)))
        for subset, sequence in group.items()
    }


def find_symmetry_groups(subset: Symmetry) -> dict[Symmetry, list[str]]:
    """Naive list of symmetries for each subset."""
    axis_symmetries = _canonicalize_group(
        {
            Symmetry.ud: [],
            Symmetry.fb: ["x"],
            Symmetry.lr: ["z"],
    ...
    face_symmetries = _canonicalize_group(
        {
            Symmetry.up: [],
            Symmetry.down: ["x2"],
            Symmetry.front: ["x'"],
    ...
    edge_symmetries = _canonicalize_group(
        {
            Symmetry.ub: [],
            Symmetry.uf: ["y2"],
            Symmetry.ul: ["y'"],
    ...
    corner_symmetries = _canonicalize_group(
        {
            Symmetry.ubl: [],
            Symmetry.ubr: ["y"],
            Symmetry.ufl: ["y'"],
    ...
    face_corner_symmetries = _canonicalize_group(
        {
            Symmetry.up_bl: [],
            Symmetry.up_br: ["y"],
            Symmetry.up_fl: ["y'"],
    ...
    face_opposite_corners_symmetries = _canonicalize_group(
        {
            Symmetry.up_bl_fr: [],
            Symmetry.up_br_fl: ["y"],
    ...
    face_edge_symmetries = _canonicalize_group(
        {
            Symmetry.up_b: [],
            Symmetry.up_f: ["y2"],
            Symmetry.up_l: ["y'"],
