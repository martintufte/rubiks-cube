...
class TestInvert:
    ...
    def test_invert_composition_identity(self) -> None:
        ...
        identity = get_identity(size=100)
        # Create a non-trivial permutation
        perm = create_random_permutation(size=100)
        inverted = invert(perm)
...
class TestMultiply:
    ...
    def test_multiply_preserves_permutation_property(self) -> None:
        base = create_random_permutation(size=10)
...
class TestConjugate:
    ...
    def test_conjugate_matches_formula(self) -> None:
        perm = create_random_permutation(size=10)
        g = create_random_permutation(size=10)
...
class TestUtilsIntegration:
    ...
    def test_all_utils_preserve_array_properties(self) -> None:
        ...
        perm = get_identity(size=4)

        # Test invert
        inverted = invert(perm)
        assert inverted.dtype == perm.dtype
        assert inverted.size == perm.size

        # Test multiply
        multiplied = multiply(perm, 2)
        assert multiplied.dtype == perm.dtype
        assert multiplied.size == perm.size

        # Test rotate_face
        conjugated = conjugate(perm, g=perm)
        assert conjugated.dtype == perm.dtype
        assert conjugated.size == perm.size
